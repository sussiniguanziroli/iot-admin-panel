...
const MainLayout = () => {
  ...
  const { isEditMode, toggleEditMode, machines, widgets, loadProfile, diagramNodes, diagramEdges } = useDashboard();
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      try {
        loadProfile(JSON.parse(event.target.result));
        setIsSettingsOpen(false);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex font-sans transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className={`flex-1 min-h-0 transition-colors duration-200 ${
          isDashboard
            ? 'overflow-hidden p-0'
            : 'overflow-y-auto p-4 md:p-8 bg-slate-50 dark:bg-slate-900'
        }`}>
          <Outlet />
        </main>
      </div>
