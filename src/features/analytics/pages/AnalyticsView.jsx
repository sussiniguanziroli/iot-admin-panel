...
import { BarChart2, ChevronDown, Search, Building2, BookMarked } from 'lucide-react';
import { useAuth } from '../../auth/context/AuthContext';
import { useAnalytics } from '../../../shared/hooks/useAnalytics';
import GlobalView from '../components/GlobalView';
import DrillDownView from '../components/DrillDownView';
import SnapshotsLibrary from '../components/SnapshotsLibrary';

const TABS = [
    { id: 'analytics', label: 'Análisis Histórico', icon: BarChart2 },
    { id: 'snapshots', label: 'Snapshots', icon: BookMarked },
];
...
const AnalyticsView = () => {
    ...
    useEffect(() => {
        if (isSuperAdmin) {
            ...
            if (userProfile?.tenantId) setSelectedTenantId(userProfile.tenantId);
        }
    ...
    const availableDataKeys = useMemo(() => {
        ...
        relevantWidgets.forEach(w => {
            if (w.dataKey && !seen.has(w.dataKey)) {
                seen.add(w.dataKey);
                keys.push({ dataKey: w.dataKey, label: w.title || w.dataKey, unit: w.unit || '', type: w.type });
            }
        });
        return keys;
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            {activeTab === 'snapshots' && (
                <SnapshotsLibrary tenantId={selectedTenantId || userProfile?.tenantId} />
            )}
