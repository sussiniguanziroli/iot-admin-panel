...
const SymbolPreview = ({ symbolKey, active, size = 52 }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ transform: 'scale(0.38)', transformOrigin: 'center', display: 'flex', flexShrink: 0 }}>
...
const EditSymbolModal = ({ isOpen, onClose, onSave, initialData }) => {
  const [symbolKey, setSymbolKey] = useState('fuse');
  const [rotation,  setRotation]  = useState(0);
  const [label,     setLabel]     = useState('');
  const [labelPos,  setLabelPos]  = useState('bottom');

  // Cargar datos iniciales del nodo
  useEffect(() => {
    if (isOpen && initialData) {
      setSymbolKey(initialData.symbolType ?? 'fuse');
      setRotation(initialData.rotation    ?? 0);
      setLabel(initialData.label          ?? '');
      setLabelPos(initialData.labelPos    ?? 'bottom');
    }
  }, [isOpen, initialData]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ symbolType: symbolKey, rotation, label: label.trim(), labelPos });
    onClose();
  };

  const PreviewSym = getSymbol(symbolKey);
        ...
        <form onSubmit={handleSubmit}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div key={groupName}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
                  {items.map(({ key, label: lbl }) => {
                    ...
                    return (
                      <button key={key} type="button" onClick={() => setSymbolKey(key)}
                        style={{
                          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                          padding: '8px 4px', borderRadius: 10, minHeight: 76,
                          border: `2px solid ${isSel ? BORDER_SEL : BORDER}`,
                          backgroundColor: isSel ? BG_SEL : BG_CARD,
                          cursor: 'pointer', transition: 'all 0.15s',
                        }}
                      >
                        <SymbolPreview symbolKey={key} active={isSel} />
                        <span style={{
            ...
            <div style={{
              ...
              padding: '14px 18px', backgroundColor: BG_CARD,
            ...
            <div>
              <label style={{
                display: 'block', fontSize: 9, fontWeight: 800, color: MUTED,
                textTransform: 'uppercase', letterSpacing: '0.1em',
            ...
            {label && (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
                  {LABEL_POSITIONS.map(({ key, label: lbl }) => {
                    const isSel = labelPos === key;
                    return (
                      <button key={key} type="button" onClick={() => setLabelPos(key)}
                        style={{
                          padding: '8px 4px', borderRadius: 8,
                          border: `1.5px solid ${isSel ? BORDER_SEL : BORDER}`,
                          backgroundColor: isSel ? BG_SEL : '#070d1a',
                          color: isSel ? '#93c5fd' : MUTED,
                          fontSize: 10, fontWeight: 700, fontFamily: 'monospace',
                          cursor: 'pointer', transition: 'all 0.15s',
                        }}
                      >
          ...
          <div style={{
            ...
            <button type="button" onClick={onClose}
              style={{
                flex: 1, padding: '11px', backgroundColor: '#0d1525',
            ...
            <button type="submit"
              style={{
                ...
                border: 'none', cursor: 'pointer', backgroundColor: '#1d4ed8',
