...
const ConnectionModal = ({ isOpen, onClose }) => {
  // 1. UPDATE: Use the new names from MqttContext
  const { connectToBroker, connectionStatus, activeConfig } = useMqtt();
  ...
  useEffect(() => {
    if (activeConfig) {
      setFormData(prev => ({ ...prev, ...activeConfig }));
    }
  }, [activeConfig]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // 2. UPDATE: Call the new function
    connectToBroker(formData); 
    onClose(); 
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95">
        ...
        <div className="bg-slate-50 px-6 py-6 border-b border-slate-100 flex justify-between items-center">
          <div className="flex gap-4 items-center">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">Conexión Manual</h2>
              <p className="text-xs text-slate-500">Override MQTT Broker</p>
            </div>
          </div>
